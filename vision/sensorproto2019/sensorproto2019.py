...
class DreadbotYellowboxSensor(BaseSensorPipeline):
    ...
    def applyNettableVals(self, nt3656):

        # TODO: only set values if they have changed
        # setHsv is method that needs to be added to a pipeline generated by GRIP
        # It should look something like this:
        '''
    def setHsv(self, hue, sat, val):
        """ Set the HSV value from network tables. hue,sat,val are arrays of min,max val pairs """
        self.__hsv_threshold_hue = hue
        self.__hsv_threshold_saturation = sat
        self.__hsv_threshold_value = val        
        '''
        hue = [ float(nt3656.hsv_h_lo), float(nt3656.hsv_h_hi) ]
        sat = [ float(nt3656.hsv_s_lo), float(nt3656.hsv_s_hi) ]
        val = [ float(nt3656.hsv_v_lo), float(nt3656.hsv_v_hi) ]
        self.pipeline.setHsv( hue, sat, val )

        with open( "hsvtry.txt", "w" ) as outf:
            outf.write( "%s\n" % [ hue, sat, val ] )

    def status(self):

        (hue, sat, val) = self.pipeline.getHsv()
...
class DreadbotAutolineSensor(BaseSensorPipeline):
    ...
    def __init__(self, name, pipeline):
        BaseSensorPipeline.__init__( self, name, pipeline )  # Class the super class's __init__ method
        self.target_zone = (0.33, 1.0)  # The autoline should be in the bottom half of the image.
        self.color = green = (0,255,0)
        #self.pipeline = pipeline
    pass

    def _process(self, image):
        self.pipeline.process( image, image )
