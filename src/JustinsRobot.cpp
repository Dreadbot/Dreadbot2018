...
public:
	...
	//SmartDashboard::PutNumber("robotPos", robotPos);
    	...
    	dt = new Drivetrain(lf,lr,rf,rr,sLift);
	}
...
//This is included in Drivetrain.h
/*
	void Wait(double t)
	{
		int wait = 0;
...
//		frc::SmartDashboard::PutNumber("Current Angle", CurrentAngle);
	...
	void RightTwo(bool a){
		...
		if (State == 0){
			dt->DrivePosition(24.0);
			State++;
		}
		else if (State == 1){
			if(dt->isdoneStraight()){
				State++;
			}
		}
		else if (State == 2){
		...
		SmartDashboard::PutNumber("StateState",State);
	}
	void LeftTwo(bool a){
		int locTime;
		int tac1 = 90;
